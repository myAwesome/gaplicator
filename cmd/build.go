...
var buildCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := validateConfig(cfg, os.Stderr); err != nil {
			return err
		}

		return runBuild(cfg, outputPath, os.Stdout)
	},
}
...
func validateConfig(cfg *generator.Config, w io.Writer) error {
	if errs := generator.ValidateConfig(cfg); len(errs) > 0 {
		if w != nil {
			fmt.Fprintf(w, "config validation failed (%d error(s)):\n", len(errs))
			for _, e := range errs {
				fmt.Fprintf(w, "  - %v\n", e)
			}
		}
...
func runBuild(cfg *generator.Config, out string, w io.Writer) error {
	var backendSteps []buildStep

	if cfg.App.Server == "node" {
		backendSteps = []buildStep{
	...
	devShStep := buildStep{"Generating dev.sh", func() error {
		var content string
		var err error
		if cfg.App.Server == "node" {
	...
	steps = append(steps, buildStep{"Generating docker-compose.yml", func() error {
	...
	steps = append(steps, buildStep{"Generating .env", func() error {
	...
	steps = append(steps, buildStep{"Generating shutdown.sh", func() error {
	...
	steps = append(steps, buildStep{"Generating README.md", func() error {
	...
	steps = append(steps, buildStep{"Generating client", func() error {
